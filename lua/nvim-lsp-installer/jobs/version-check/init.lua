...
local version_checker = {
    ["npm"] = function(server, receipt)
        local ok, stdout = spawn.npm {
            "ls",
            "--json",
            cwd = server.root_dir,
        }
        assert(ok, "Failed to spawn npm.")
    ...
    ["pip3"] = function(server, receipt)
        ...
        assert(ok, "Failed to spawn pip3.")
    ...
    ["gem"] = function(server, receipt)
        local ok, stdout = spawn.gem {
            "list",
            cwd = server.root_dir,
            env = process.graft_env(gem.env(server.root_dir)),
        }
        assert(ok, "Failed to spawn gem.")
        local gems = parse_gem_list_output(stdout)
    ...
    ["cargo"] = function(server, receipt)
        ...
        assert(ok, "Failed to spawn cargo.")
    ...
    ["git"] = function(server)
        local ok, stdout = spawn.git {
            "rev-parse",
            "--short",
            "HEAD",
            cwd = server.root_dir,
        }
        assert(ok, "Failed to spawn git.")
...
function M.check_server_version(server)
    ...
    return version_check(server, receipt)
end
